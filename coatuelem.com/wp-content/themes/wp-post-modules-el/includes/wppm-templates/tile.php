 ...
 */
	...
	if ( ! $enable_slider ) {
		$out = sprintf( '%s<div%s%s class="wppm wppm-tile clearfix%s">',
...
/*		// Image resize
		...
		if ( has_post_thumbnail() ) {
			...
			if ( ( 'video' == get_post_format() || 'gallery' == get_post_format() ) && $post_format_icon ) {
				$format_icon = '<div class="' . get_post_format() . '-overlay"></div>';
			}

			$thumblink = sprintf( apply_filters( 'wppm_portfolio_thumbnail', '<div%5$s class="post-img%7$s"><a href="%1$s" title="%2$s">%3$s%4$s</a>%6$s</div>' ),
		...
		if ( ( 'featured' == $img_source && has_post_thumbnail() ) || $img_url ) {
			$out .= sprintf ( $format,
				$post_classes,
				$thumblink,
