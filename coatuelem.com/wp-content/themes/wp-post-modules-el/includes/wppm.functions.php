	...
	function wppm_el_meta( $args = array() ) {
		...
		$custom_link = get_post_meta( $post->ID, 'wppm_el_custom_link', true );
			...
			if ( $review_meta != '' || $cat_meta != '' ) {
				...
				( ! $show_cats && ! $show_reviews && ! $show_date && ! $show_author && ! $show_views && ! $show_comments && 'true' !== $sharing ) ? ' schema-only' : ''
			);
				if ( $cat_meta != '' ) {
			...
			$row_4 .= sprintf( '<aside class="meta-row row-3%s">',
				( ! $show_date && ! $show_author && ! $show_views && ! $show_comments && 'true' !== $sharing ) ? ' schema-only' : ''
			);
			...
			if ( $readmore ) {
				if ( $meta_data ) {
					$row_2 = sprintf( '<aside class="meta-row row-2%s"><div class="meta-col">%s</div></aside>',
						( ! $show_date && ! $show_author && ! $show_views && ! $show_comments ) ? ' schema-only' : '',
						$meta_data
					);
				}
		...
		if ( ! empty( $share_btns ) && is_array( $share_btns ) ) {
			foreach ( $share_btns as $button ) {
				...
				switch( $button ) {
					...
					case 'twitter':
						$list .= sprintf( '<li class="wppm-twitter"><a href="%s://twitter.com/home?status=%s" target="_blank" title="%s">%s</a></li>', esc_attr( $protocol ), urlencode( esc_url( get_permalink() ) ), esc_attr__( 'Share on twitter', 'wppm-el' ), esc_attr__( 'Twitter', 'wppm-el' ) );
					break;
...
function wppm_el_sharing_buttons( $sharing_buttons, $limit, $text = false ) {
	global $post;
		...
		foreach ( $sharing_buttons as $button ) {
				switch( $button ) {
					case 'twitter':
						$list .= sprintf( '<li class="nn-twitter"><a href="%s://twitter.com/home?status=%s" target="_blank" title="%s"><i class="fab fa-twitter"></i>%s</a></li>', esc_attr( $protocol ), urlencode( esc_url( get_permalink() ) ), esc_attr__( 'Share on twitter', 'wppm_el' ), $text ? esc_attr__( 'Twitter', 'wppm_el' ) : '<span class="sr-only">' . esc_attr__( 'Twitter', 'wppm_el' ) . '</span>'  );
					break;
...
if ( ! function_exists( 'wppm_el_social_sharing' ) ) :
	function wppm_el_social_sharing( $sharing_buttons, $share_style = 'popup' ) {
		...
		$out .= sprintf( '<div class="sharing-overlay"><ul class="wppm-el-sharing-list clearfix"><li class="sharing-modal-handle no-popup">%s<a class="close-sharing" href="#" title="%s"><span class="screen-reader-text">%s</span></a></li><li class="share-post-title">%s</li>',
			esc_attr__( 'Share this post', 'wppm_el' ),
...
function qalam_sharing_buttons( $sharing_buttons, $limit, $text = false ) {
	global $post;
		...
		foreach ( $sharing_buttons as $button ) {
			switch( $button ) {
				case 'twitter':
					$list .= sprintf( '<li class="qlm-twitter"><a href="%s://twitter.com/home?status=%s" target="_blank" title="%s"><i class="fab fa-twitter"></i>%s</a></li>', esc_attr( $protocol ), urlencode( esc_url( get_permalink() ) ), esc_attr__( 'Share on twitter', 'qalam' ), $text ? esc_attr__( 'Twitter', 'qalam' ) : '<span class="sr-only">' . esc_attr__( 'Twitter', 'qalam' ) . '</span>'  );
				break;
...
if ( ! function_exists( 'qalam_social_sharing' ) ) :
	function qalam_social_sharing( $sharing_buttons, $extra = 'true' ) {
		...
		$out .= sprintf( '<div class="sharing-overlay"><ul class="qlm-sharing-list clearfix"><li class="sharing-modal-handle no-popup">%s<a class="close-sharing" href="#" title="%s"><span class="screen-reader-text">%s</span></a></li><li class="share-post-title">%s</li>',
			esc_attr__( 'Share this post', 'qalam' ),
