...
class Instagram_Feed extends Base {
	...
	protected function register_content_controls() {
		...
		$this->add_control(
			'user_id',
			[
				'label' => __('User ID', 'happy-addons-pro'),
		...
		$this->add_control(
			...
			[
				...
				'raw' => '<a href="https://developers.facebook.com/docs/instagram-basic-display-api/getting-started" target="_blank">Get User ID & Access Token</a>',
		...
		$this->add_control(
			'cash_headding',
			[
				'label' => __( 'Data Cash settings', 'happy-addons-pro' ),
				'label_block' => true,
		...
		$this->add_control(
			'cash_time',
			[
				'label' => __('Data Cash Time', 'happy-addons-pro'),
				//'description' => __( "Default value is 1. Set 0 to remove cash immediately.", 'happy-addons-pro' ),
		...
		$this->add_control(
			'cash_note',
			[
				'label' => __( 'Cash Related Nonte', 'happy-addons-pro'),
				'show_label' => false,
				'type' => \Elementor\Controls_Manager::RAW_HTML,
				'raw' => __( "Default value is 1. Set 0 to remove cash immediately.", 'happy-addons-pro' ),
		...
		$this->add_control(
			'cash_time_type',
			[
				'label' => __('Data Cash Time Type', 'happy-addons-pro'),
	...
	protected function instafeed_render() {
		...
		$cash_time = 0 === $settings['cash_time'] || !empty($settings['cash_time']) ? $settings['cash_time'] : 1;
		$cash_time_type = $this->get_cashing_time_type($settings['cash_time_type']);
		//var_dump($cash_time);
		$item_tag = 'div';
		...
		if ( '0' == $cash_time ) {
			...
			$cash_time = 1;
			$cash_time_type = MINUTE_IN_SECONDS;
		}
		...
		if (false === $instagram_data) {
			...
			//echo 'Data cahed';
			set_transient($transient_key, $instagram_data, $cash_time * $cash_time_type); //HOUR_IN_SECONDS
		}
		...
		if ( empty($user_id) && empty($access_token) ) {
			$messages['invalid_token_id'] = __('Please Input User ID and Access token', 'happy-addons-pro');
		}elseif ( empty($user_id) || empty($access_token) || empty($instagram_data) ) {
			$messages['invalid_token_id'] = __('Please Input valid User ID and Access token', 'happy-addons-pro');
	...
	protected function get_cashing_time_type($type){
		switch ( $type ) {
			case 'minute':
				$cash_time_type = MINUTE_IN_SECONDS;
				break;
			case 'hour':
				$cash_time_type = HOUR_IN_SECONDS;
				break;
			case 'day':
				$cash_time_type = DAY_IN_SECONDS;
				break;
			case 'week':
				$cash_time_type = WEEK_IN_SECONDS;
				break;
			case 'month':
				$cash_time_type = MONTH_IN_SECONDS;
				break;
			case 'year':
				$cash_time_type = YEAR_IN_SECONDS;
				break;
			default:
				$cash_time_type = DAY_IN_SECONDS;
		}
		return $cash_time_type;
	}
